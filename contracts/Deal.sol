...
contract Deal {
    ...
    constructor(address[] memory _owners) {
        ...
        products["Skateboard"] = Product({ price: 2 ether, token: ERC20Tokens["NULL"], isBroken: false, productOwner: msg.sender });
    }
   ...
   function getInvoiceInfo(uint invoiceIndex) public view returns(address, address payable, string memory, uint, uint) {
       require(invoiceIndex < invoicesCount, "There is no invoice for this index");
       
       return (invoices[invoiceIndex].buyer, invoices[invoiceIndex].seller, invoices[invoiceIndex].productName, invoices[invoiceIndex].price, invoices[invoiceIndex].date);
   } 
//   ["0x5B38Da6a701c568545dCfcB03FcB875f56beddC4","0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2","0xCA35b7d915458EF540aDe6068dFe2F44E8fa733c"]
   ...
   function getData() public view returns(bytes memory){
       return abi.encodeWithSignature("changeProductOwner(uint)", invoicesCount);
   }
   
   function changeProductOwner(uint invoiceIndex) external {
      (,,,,uint numConfirmations) = multiSigWallet.getTransaction(invoices[invoiceIndex].transactionIndex);
      
      require(numConfirmations == 2 && !invoices[invoiceIndex].isConfirmed, "The function is called only when the transaction is confirmed by at least two owners and invoice has not been confirmed before");
      
      invoices[invoiceIndex].isConfirmed = true;
      products[invoices[invoiceIndex].productName].productOwner = invoices[invoiceIndex].buyer;
    //   invoices[invoiceIndex].seller.transfer(products[invoices[invoiceIndex].productName].price);
  }
     ...
     function buyProduct(string memory productName) payable public {
       ...
       if(products[productName].token.tokenOwner == address(0)){
           ...
           bytes memory transactionData = abi.encodeWithSignature("changeProductOwner(uint)", invoicesCount);
           
            multiSigWallet.submitTransaction(msg.sender, address(this), 0, transactionData); // submit transfer eather from buyer to seller
            multiSigWallet.confirmTransaction(msg.sender, invoices[invoicesCount].transactionIndex);
    ...
    function confirmProductSale(uint invoiceIndex) public {
        require(invoices[invoiceIndex].seller == msg.sender, "In order to confirm the product sale, it is necessary to be a seller of the product");
        
        multiSigWallet.confirmTransaction(msg.sender, invoices[invoiceIndex].transactionIndex);
        multiSigWallet.executeTransaction(msg.sender, invoices[invoiceIndex].transactionIndex);
    }
    ...
    function resolveComplaint(uint complaintIndex, string memory comment, bool acceptPayment) public {
        require(complaintIndex < complainsCount, "There is no complaint for this index");
        ...
        if(acceptPayment){
            multiSigWallet.confirmTransaction(msg.sender, complains[complaintIndex].invoice.transactionIndex);
            multiSigWallet.executeTransaction(msg.sender, complains[complaintIndex].invoice.transactionIndex);
        }
